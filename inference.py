...
from models.aaai.model_inference import MMModerator  # Your model definition
import os
...
def extract_and_crop_frames(video_path: str,
    ...
    frames = vr.get_batch(range(len(vr))).asnumpy()  # (F, H, W, 3)
...
def inference(video_path: str, checkpoint: str, device: torch.device, model , tokenizer):
    ...
    with torch.no_grad():
        logits, *_ = model(
            mfcc=mfcc[:30],
            mfcc_aug=mfcc_aug[:30],
            audio=None,
            video=video[:30],
            video_aug=video_aug[:30],
            text=None,
            landmarks=None,
            flow=None,
            images=None,
        ...
        return 1 if (scores.mean()<0.54) else 0 
...
if __name__ == "__main__":
    ...
    for video in os.listdir(args.video_path):
        path = os.path.join(args.video_path , video)
        if "mp4" in video:
            try:
                print(f"Processing: video {video}")
                result = inference(path, args.checkpoint, device, model,tokenizer)
                correct += result 
                total += 1
                ...
                continue
    print(f"Total {correct} are correct out of {total}")
        # python inference.py --video_path /media/NAS/DATASET/FaceForensics_Dec2020/FaceForensics++/video_data/data_raw/manipulated_sequences/NeuralTextures/raw/cropped_faces/001_870.avi --checkpoint 

        # python inference.py --checkpoint /media/NAS/USERS/shahid/MultimodalAudioVisualModerator/training_logs/aaai/multimodal/transfer_learning_audio_visual/rawnet/20250703-194311/checkpoints/model_state.pt  --video_path /media/NAS/USERS/shahid/MultimodalAudioVisualModerator/deepfakes/reals